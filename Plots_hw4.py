...
datos = np.genfromtxt("datosProyectil.txt", delimiter = " ", usecols = (1,2))
posiciones = []

for i in range(len(datos[:,0])):
	if(datos[i,0] == 0.0 and datos[i,1] == 0.0):
		posiciones.append(i)
plt.figure()
for i in range(len(posiciones) - 1):
	arregloX = datos[posiciones[i]:posiciones[i+1],0]
	arregloY = datos[posiciones[i]:posiciones[i+1],1]
	plt.plot(arregloX, arregloY, label = "Angulo " + str(10*(i+1)))
...
plt.figure()
datos45X = datos[posiciones[-1]:len(datos[:,0]) -1,0]
datos45Y = datos[posiciones[-1]:len(datos[:,0]) -1,1]
plt.plot(datos45X, datos45Y, label = "Angulo 45")
plt.legend()
